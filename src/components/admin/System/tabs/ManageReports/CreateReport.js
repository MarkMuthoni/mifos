...
const ReportForm = () => {
    ...
    const fetchReportTemplate = async () => {
        startLoading();
        try {
            const response = await fetch(`${API_CONFIG.proxy}/fineract-provider/api/v1/reports/template`, {
                method: 'GET',
                headers: {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_CONFIG.proxy}/fineract-provider/api/v1/reports`, {
                method: 'POST',
                headers: {
                    Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                    'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
