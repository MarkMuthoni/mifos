...
const CreateUserForm = ({ onUserCreated }) => {
    ...
    const fetchTemplateData = async () => {
        try {
            const response = await axios.get(`${API_CONFIG.proxy}/fineract-provider/api/v1/users/template`, {
                headers: {
            ...
            const officesResponse = await axios.get(`${API_CONFIG.proxy}/fineract-provider/api/v1/offices`, {
                headers: {
                    Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                    'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
                    "Content-Type": "application/json",
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.post(`${API_CONFIG.proxy}/fineract-provider/api/v1/users`, payload, {
                headers: {
                    Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                    'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
                    "Content-Type": "application/json",
