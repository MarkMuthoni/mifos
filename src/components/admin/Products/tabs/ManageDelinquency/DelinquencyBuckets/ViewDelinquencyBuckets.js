...
const ViewDelinquencyBuckets = () => {
    ...
    const fetchDelinquencyBuckets = async () => {
        startLoading();
        try {
            const response = await axios.get(`${API_CONFIG.proxy}/fineract-provider/api/v1/delinquency/buckets`, {
                headers: {
    ...
    const handleRowClick = async (bucket) => {
        ...
        try {
            const response = await axios.get(
                `${API_CONFIG.proxy}/fineract-provider/api/v1/delinquency/buckets/${bucket.id}`,
                {
                    headers: {
                        Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                        'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
    ...
    const handleBucketModalSubmit = async () => {
        ...
        try {
            ...
            await axios.put(
                `${API_CONFIG.proxy}/fineract-provider/api/v1/delinquency/buckets/${selectedBucket.id}`,
                payload,
                {
                    headers: {
                        Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
