...
const ViewEntityDataTableChecks = () => {
    ...
    const fetchDataTableChecks = async () => {
        startLoading();
        try {
            const response = await axios.get(`${API_CONFIG.proxy}/fineract-provider/api/v1/entityDatatableChecks?offset=0&limit=-1`, {
                headers: {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this data table check?")) {
            startLoading();
            try {
                await axios.delete(`${API_CONFIG.proxy}/fineract-provider/api/v1/entityDatatableChecks/${id}`, {
                    headers: {
                        Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                        'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
                        'Content-Type': 'application/json',
