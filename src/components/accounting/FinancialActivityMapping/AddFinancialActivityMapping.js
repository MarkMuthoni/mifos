...
const AddFinancialActivityMappingForm = () => {
    ...
    useEffect(() => {
        const fetchTemplateData = async () => {
            startLoading();
            try {
                const response = await axios.get(`${API_CONFIG.proxy}/fineract-provider/api/v1/financialactivityaccounts/template`, {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${API_CONFIG.proxy}/fineract-provider/api/v1/financialactivityaccounts`, newMappingData, {
                headers: {
                    Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                    'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
                    'Content-Type': 'application/json',
            ...
            const mappingDetailsResponse = await axios.get(
                `${API_CONFIG.proxy}/fineract-provider/api/v1/financialactivityaccounts/${mappingId}?template=false`,
                {
                    headers: {
                        Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                        'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            await axios.put(`${API_CONFIG.proxy}/fineract-provider/api/v1/financialactivityaccounts/${mappingDetails.id}`, updatedMappingData, {
                headers: {
                    Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                    'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
                    'Content-Type': 'application/json',
            ...
            const updatedMappingDetailsResponse = await axios.get(
                `${API_CONFIG.proxy}/fineract-provider/api/v1/financialactivityaccounts/${mappingDetails.id}?template=false`,
                {
                    headers: {
                        Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                        'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this mapping?')) {
            startLoading();
            try {
                await axios.delete(`${API_CONFIG.proxy}/fineract-provider/api/v1/financialactivityaccounts/${mappingDetails.id}`, {
                    headers: {
                        Authorization: `Basic ${user.base64EncodedAuthenticationKey}`,
                        'Fineract-Platform-TenantId': `${API_CONFIG.tenantId}`,
                    },
